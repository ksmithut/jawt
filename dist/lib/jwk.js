...
export async function importJWK(jwk, algorithm) {
    return webcrypto.subtle.importKey('jwk', jwk, subtleDSA(algorithm), true, keyOps(jwk));
}
/**
 * @param {JsonWebKey} jwk
 * @returns {JsonWebKey}
...
export async function generateKid(jwk) {
    ...
    return base64urlEncode(await webcrypto.subtle.digest('SHA-256', stringToArrayBuffer(JSON.stringify(strippedJWK))));
}
