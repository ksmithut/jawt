import { base64urlEncode, base64urlDecode, base64urlDecodeToArrayBuffer } from './base64.node.js';
import { stringToArrayBuffer } from './utils.js';
import * as v from './validate.js';
import * as jws from './jws.js';
import { isAlgorithm } from './jwa.js';
import { AlgorithmMismatch, InvalidJSON, InvalidAlgorithm, InvalidKeyId, InvalidSignature, MalformedJWT } from './jwt.errors.js';
/**
...
export async function signJWT(header, payload, cryptoKey) {
    ...
    const signature = base64urlEncode(await jws.sign(header.alg, cryptoKey, stringToArrayBuffer(body)));
...
export function decodeJWT(token) {
    if (!v.isString(token))
        throw new MalformedJWT('JWT not a string');
    const parts = token.trim().split('.');
    if (parts.length !== 3)
        throw new MalformedJWT('Not in valid JWT format');
    ...
    if (typ !== 'JWT')
        throw new MalformedJWT('JWT header missing "typ":"JWT"');
    if (!isAlgorithm(alg))
        throw new InvalidAlgorithm();
    if (!v.optional(v.isString)(kid))
