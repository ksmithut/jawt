 ...
 * @returns {PayloadWithStandardClaims}
 */
export function attachStandardClaims(payload, { clock = new Date(), issuer, subject, audience, expiresAt, expiresIn, notBefore, issuedAt = true, jwtId } = {}) {
    if (!v.isPlainObject(payload)) {
        throw new TypeError('"payload" must be a plain object');
    }
    if (!v.isDate(clock))
        throw new TypeError('"clock" must be a Date object');
    ...
    if (expiresIn !== undefined)
        ...
        claims.exp = dateToTimestamp(expiresAt);
    else if (expiresAt !== undefined)
        claims.exp = expiresAt;
    if (notBefore instanceof Date)
        claims.nbf = dateToTimestamp(notBefore);
    else if (notBefore !== undefined)
        claims.nbf = notBefore;
    if (issuedAt === true)
        claims.iat = now;
    else if (issuedAt instanceof Date)
        claims.iat = dateToTimestamp(issuedAt);
    else if (issuedAt !== undefined && issuedAt !== false)
        claims.iat = issuedAt;
    if (jwtId !== undefined)
        claims.jti = jwtId;
 ...
 */
export function verifyStandardClaims(payload, { clock = new Date(), clockTolerance = 0, maxAge = Infinity, issuer, subject, audience, jwtId } = {}) {
    const now = dateToTimestamp(clock);
    ...
    if (issuer != null) {
        if (!toArray(issuer).some(issuer => issuer === claims.iss)) {
            throw new InvalidClaim('iss', '"iss" did not match expected value', claims.iss, issuer);
        }
    }
    if (subject != null) {
        if (subject !== claims.sub) {
            throw new InvalidClaim('sub', '"sub" did not match expected value', claims.sub, subject);
        }
    }
    if (audience != null) {
        ...
        if (!validAudience) {
            throw new InvalidClaim('aud', '"aud" did not match expected value', claims.aud, audience);
        }
    }
    if (jwtId != null) {
        if (jwtId !== claims.jti) {
            throw new InvalidClaim('jti', '"jti" did not match expected value', claims.jti, jwtId);
        }
    }
    return claims;
}
...
//# sourceMappingURL=jwt.standard-claims.js.map