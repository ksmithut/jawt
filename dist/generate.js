...
/** @type {{ modulusLength?: number } | undefined} */ options) {
    switch (alg) {
        ...
        case 'PS256':
            return generatePS(256, { modulusLength: options?.modulusLength }).then(key => createKeyfromCryptoKey(key, { alg }));
        case 'PS384':
            return generatePS(384, { modulusLength: options?.modulusLength }).then(key => createKeyfromCryptoKey(key, { alg }));
        case 'PS512':
            return generatePS(512, { modulusLength: options?.modulusLength }).then(key => createKeyfromCryptoKey(key, { alg }));
        case 'RS256':
            return generateRS(256, { modulusLength: options?.modulusLength }).then(key => createKeyfromCryptoKey(key, { alg }));
        case 'RS384':
            return generateRS(384, { modulusLength: options?.modulusLength }).then(key => createKeyfromCryptoKey(key, { alg }));
        case 'RS512':
            return generateRS(512, { modulusLength: options?.modulusLength }).then(key => createKeyfromCryptoKey(key, { alg }));
        case 'ES256':
            return generateES('P-256').then(key => createKeyfromCryptoKey(key, { alg }));
        case 'ES384':
            return generateES('P-384').then(key => createKeyfromCryptoKey(key, { alg }));
        case 'ES512':
            return generateES('P-521').then(key => createKeyfromCryptoKey(key, { alg })); // yes 521
        case 'ES256K':
        case 'EdDSA':
        default:
