import { base64urlEncode, base64urlDecode, stringToArrayBuffer } from './lib/utils/encoding.js';
import { InvalidKeyStore, MalformedJWT, InvalidKeyId, InvalidAlgorithm, AlgorithmMismatch, InvalidSignature, InvalidClaim, TokenExpired, NotBefore, IssuerNotAccepted, AgeNotAccepted, AudienceNotAccepted, SubjectNotAccepted, JwtIdNotAccepted, InvalidJSON } from './lib/errors.js';
 ...
 */
export async function sign(payload, keyStore, { now = new Date(), issuer, subject, audience, expiresAt, expiresIn, notBefore, issuedAt = true, jwtId } = {}) {
    ...
    if (expiresAt instanceof Date)
        payload.exp = dateToTimestamp(expiresAt);
    else if (typeof expiresAt === 'number')
        payload.exp = expiresAt;
    else if (typeof expiresIn === 'number')
        payload.exp = nowTimestamp + expiresIn;
    if (notBefore instanceof Date)
        payload.nbf = dateToTimestamp(notBefore);
    else if (typeof notBefore === 'number')
        payload.nbf = notBefore;
    if (issuedAt === true)
        payload.iat = nowTimestamp;
    else if (issuedAt instanceof Date)
        payload.iat = dateToTimestamp(issuedAt);
 ...
 */
export async function verify(token, keyStore, { now = new Date(), issuer, subject, audience, jwtId, clockTolerance, maxAge } = {}) {
    if (!isKeyStore(keyStore))
        throw new InvalidKeyStore();
    ...
    if (issuer !== undefined)
        validateIssuer(issuer, payload.iss);
    if (subject !== undefined)
        validateSubject(subject, payload.sub);
    if (audience !== undefined)
        validateAudience(audience, payload.aud);
    if (jwtId !== undefined)
        validateJwtId(jwtId, payload.jti);
...
function validateIssuer(expected, given) {
    if (typeof given !== 'string')
        throw new InvalidClaim('iss');
    expected = Array.isArray(expected) ? expected : [expected];
    if (!expected.includes(given))
        throw new IssuerNotAccepted();
}
/**
...
function validateSubject(expected, given) {
    if (typeof given !== 'string')
        throw new InvalidClaim('sub');
    if (expected !== given)
        throw new SubjectNotAccepted();
}
/**
 * @param {string} expected
 * @param {unknown} given
...
function validateJwtId(expected, given) {
    if (typeof given !== 'string')
        throw new InvalidClaim('jti');
    if (expected !== given)
        throw new JwtIdNotAccepted();
}
/**
...
function validateTime({ iat, exp, nbf }, { now, maxAge, clockTolerance = 0 }) {
    ...
    if (nbf !== undefined) {
        if (typeof nbf !== 'number')
            throw new InvalidClaim('nbf');
        if (nbf > nowTimestamp + clockTolerance)
            throw new NotBefore();
    }
    if (exp !== undefined) {
        if (typeof exp !== 'number')
            throw new InvalidClaim('exp');
        if (exp < nowTimestamp - clockTolerance)
            throw new TokenExpired();
    }
    if (maxAge != null) {
        if (typeof iat !== 'number')
            throw new InvalidClaim('iat');
        const age = nowTimestamp - iat;
        if (maxAge < age - clockTolerance)
            throw new AgeNotAccepted();
    }
    if (iat !== undefined) {
        if (typeof iat !== 'number')
            throw new InvalidClaim('iat');
