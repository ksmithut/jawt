...
import { UnsupportedAlgorithm, MissingAlgorithm, InvalidSigningKey } from './lib/errors.js';
import { cryptoKeyToJWK, jwkToCryptoKey, privateToPublic, generateKid, keyOps } from './lib/jwk.js';
...
export async function createKeyfromCryptoKey(cryptoKey, options) {
    ...
    const key = Object.freeze({
        ...
        jwk(priv = false) {
            if (!priv)
                return { ...clone(publicJWK), kid, alg: algorithm };
...
export async function createKeyFromJWK(jwk) {
    if (!jwk.alg)
        throw new MissingAlgorithm();
    if (!isAlgorithm(jwk.alg))
        throw new UnsupportedAlgorithm(jwk.alg);
    const key = await webcrypto.subtle.importKey('jwk', jwk, subtleDSA(jwk.alg), true, keyOps(jwk));
