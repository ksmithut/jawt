...
export async function sign (payload, keyStore, options) {
  if (!isKeyStore(keyStore)) throw new TypeError('Invalid KeyStore')
...
export async function verify (token, keyStore, options) {
  if (!isKeyStore(keyStore)) throw new TypeError('Invalid KeyStore')
  const [payload, header] = await jwt.verifyJWT(
    token,
    /**
    ...
    function * getKeys (header) {
      ...
      if (key) {
        const alg = key.alg()
        if (alg === header.alg) yield [alg, key.verifyingKey()]
        return
      }
      for (const key of keyStore.keys()) {
        const alg = key.alg()
        if (alg !== header.alg) continue
